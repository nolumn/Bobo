...
runFunction(function()
    ...
    billboard.StudsOffset = Vector3.new(0, -0.2500000298023224, 0.6100000143051147)
...
runFunction(function()
    ...
    bladePart.Transparency = 1
    bladePart.Size = Vector3.new(0.1, 0.57, 0.1)
    bladePart.Material = Enum.Material.Neon
    ...
    local BladeToggle = render:CreateToggle({
        ...
        Flag = 'blade_toggle',
        Callback = function(val)
            if not val then bladePart.Transparency = 1 end
        end,
    })
    ...
    cleanup.add(runService.RenderStepped:Connect(function()
        if not BladeToggle.CurrentValue or not lplr.axe then bladePart.Transparency = 1 return end
        bladePart.Transparency = 0
        bladePart.Size = Vector3.new(0.1, 0.57, 0.1)
        bladePart.CFrame = lplr.axe.CFrame * CFrame.new(0.59, -0.08, 0) * CFrame.Angles(0, 0, 0)
        local mode = BladeModeDropdown.CurrentOption[1]
        bladePart.Color = mode == 'Rainbow' and returnRainbow()
            or mode == 'Ball Color' and (lplr.ball and lplr.ball.Color or Color3.new(1,1,1))
            or BladeColorPicker.Color
        pcall(function() bladePart.Material = Enum.Material[BladeMaterialDropdown.CurrentOption[1]] end)
    end))

    cleanup.add({ Destroy = function() bladePart:Destroy() end })
end)
