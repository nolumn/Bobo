...
runFunction(function()
    ...
    local AntiKnockback = blatant:CreateToggle({
        Name = "AntiKnockback", CurrentValue = false, Flag = "anti_knockback",
        Callback = function(val)
            if not val then lastVel = Vector3.new(0, 0, 0) end
        end
    })
    local AntiKnockbackThreshold = blatant:CreateSlider({
        Name = "Spike Threshold", Range = {5, 100}, Increment = 1, Suffix = "",
        CurrentValue = 25, Flag = "anti_knockback_threshold", Callback = function() end
    })

    cleanup.add(runService.RenderStepped:Connect(function()
        if not AntiKnockback.CurrentValue then return end
        if not lplr.ball then return end
        local vel = lplr.ball.AssemblyLinearVelocity
        local delta = (vel - lastVel).Magnitude
        if delta > AntiKnockbackThreshold.CurrentValue then
            lplr.ball.AssemblyLinearVelocity = lastVel
        else
            lastVel = vel
        end
    end))
